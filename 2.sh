...
echo "1. Настройка физического интерфейса emp0s3:"
...
sudo nmcli con add type ethernet ifname emp0s3 con-name static-emp0s3 
sudo nmcli con modify static-emp0s3 ipv4.addresses 10.100.0.2/24
sudo nmcli con modify static-emp0s3 ipv4.gateway 10.100.0.1
sudo nmcli con modify static-emp0s3 ipv4.dns 8.8.8.8
# ipv4.method — метод получения IP. manual — статическая настройка (не DHCP)
sudo nmcli con modify static-emp0s3 ipv4.method manual
sudo nmcli con up static-emp0s3
...
ping -c 4 -I emp0s3 10.100.0.3 # реальный интерфейс
